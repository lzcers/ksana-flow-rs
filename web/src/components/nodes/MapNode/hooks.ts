import { useEffect, useState } from 'react';
import { useStore } from '../../../store';

export type MapNodeStreamState = {
  receivedItems: number;
  lastItemIndex: number | null;
  lastItemOutput: any;
  doneCount: number | null;
  finalCount: number | null;
};

const INITIAL: MapNodeStreamState = {
  receivedItems: 0,
  lastItemIndex: null,
  lastItemOutput: null,
  doneCount: null,
  finalCount: null,
};

export function useMapNodeStream(nodeId: string): MapNodeStreamState {
  const eventsForNode$ = useStore((s) => s.eventsForNode$);
  const [state, setState] = useState<MapNodeStreamState>(INITIAL);

  useEffect(() => {
    const stream$ = eventsForNode$?.(nodeId);
    if (!stream$) return;

    const subscription = stream$.subscribe((wrapper: any) => {
      const { event } = wrapper;

      if (event.NodeStarted === nodeId) {
        setState(INITIAL);
        return;
      }

      if (event.NodeError && Array.isArray(event.NodeError) && event.NodeError[0] === nodeId) {
        setState(INITIAL);
        return;
      }

      if (event.NodeStreamNextMessage) {
        const [id, value] = event.NodeStreamNextMessage;
        if (id !== nodeId || value == null || typeof value !== 'object') return;

        const kind = (value as any).kind;
        if (kind === 'item') {
          const index = typeof (value as any).index === 'number' ? (value as any).index : null;
          const output = (value as any).output;
          setState((prev) => ({
            ...prev,
            receivedItems: prev.receivedItems + 1,
            lastItemIndex: index,
            lastItemOutput: output,
          }));
        } else if (kind === 'done') {
          const count = typeof (value as any).count === 'number' ? (value as any).count : null;
          setState((prev) => ({
            ...prev,
            doneCount: count,
          }));
        }
        return;
      }

      if (event.NodeOutMessage) {
        const [id, value] = event.NodeOutMessage;
        if (id !== nodeId) return;
        if (Array.isArray(value)) {
          setState((prev) => ({
            ...prev,
            finalCount: value.length,
          }));
        }
      }
    });

    return () => subscription.unsubscribe();
  }, [eventsForNode$, nodeId]);

  return state;
}

