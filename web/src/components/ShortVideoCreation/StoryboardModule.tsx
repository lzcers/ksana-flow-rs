...
export const StoryboardModule: React.FC<StoryboardModuleProps> = ({ shots }) => {
  return (
    <div className="w-full h-full overflow-auto bg-white dark:bg-zinc-950 custom-scrollbar">
      <table className="w-full text-left border-collapse min-w-[1200px]">
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
          {shots.map((shot) => (
            <tr key={shot.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors text-sm text-zinc-700 dark:text-zinc-300">
              <td className="p-4 text-center font-medium">{shot.shotNo}</td>
              <td className="p-4">
