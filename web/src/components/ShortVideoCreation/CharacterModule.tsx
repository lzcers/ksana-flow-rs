...
export const CharacterModule: React.FC<CharacterModuleProps> = ({ characters }) => {
  return (
    <div className="w-full h-full overflow-auto bg-white dark:bg-zinc-950 custom-scrollbar">
      <table className="w-full text-left border-collapse">
        ...
        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-900">
          {characters.map((char) => (
            <tr key={char.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors text-sm text-zinc-700 dark:text-zinc-300">
              <td className="p-4 text-zinc-500">{char.id}</td>
              <td className="p-4">
