...
export const WorkflowHeader: React.FC<WorkflowHeaderProps> = ({
  ...
  const handleSave = () => {
    if (currentWorkflowId) {
      ...
      // If we have a custom name set via rename (on the tab), use it?
      // Actually rename on unsaved tab just updates local tab name in UI?
      // No, renameWorkflow calls backend.
      // But for unsaved workflow, renameWorkflow might fail or we need to handle it.
      // Current renameWorkflow implementation expects ID.
      // If ID is null, we can't call renameWorkflow.
      // So editing name on New Workflow tab should just update local state or trigger save?
      // For simplicity, "Save" button triggers dialog for new workflow.
