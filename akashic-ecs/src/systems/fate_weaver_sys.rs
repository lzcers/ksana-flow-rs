...
pub fn fate_weaver_system(
    ...
    fate_weaver.append_user_message(&json!({"protagonist_action": protagonist_action}).to_string());
...
pub fn fate_weaver_apply_system(
    ...
    match task_result.status {
        TaskStatus::Done => {
            ...
            let world_snapshot = parse_json_response::<WorldSnapshot>(&raw_output)
                .unwrap_or_else(|err| panic!("解析 FateWeaver 输出失败: {err}"));
            let ledger = world_snapshot.to_ledger();
