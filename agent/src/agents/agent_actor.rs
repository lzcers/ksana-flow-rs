    ...
    E: ToolExecutor + Send,
{
    ...
    pub async fn run_step(
        ...
        if let Some(dur) = timeout {
            match tokio::time::timeout(dur, self.run_step_inner(event_tx)).await {
                Ok(result) => result,
                Err(_) => {
                    self.state.state = crate::agents::JobState::Failed;
                    StepResult::Error(AgentError::Model("Timeout".into()))
                }
            }
        } else {
    ...
    pub fn run_loop(mut self) -> AgentActorHandle {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                // 检查是否已取消
                if loop_state == LoopState::Cancelled {
                    Self::send_event(
                        Some(&event_tx),
                        AgentActorEvent::Error(AgentError::Cancelled),
                    )
                    .await;
                    break;
                }
                ...
                // 执行单步迭代
                let result = self.run_step(Some(event_tx.clone()), None).await;
